...
int main(int argc, char* argv[])
{
    ...
    int data_num = 0;
    int datalist_size = MB;
    int rec_len = 0;
    struct protocol **datalist = (struct protocol **)calloc(MB,sizeof(struct protocol *));
    ...
    while(send_len > 0)
    {
        ...
        while(rec_len > 0)
        {
            struct protocol *data = (struct protocol *)calloc(rec_len+8,sizeof(char));
            int rev = recv(client_server,data,rec_len,0);
            if(ntohs(data->length) < rev)
            {
                /* 보낼려는 것보다 더 많이 온 경우 */
                /* 하나의 메세지에 헤더가 2개 포함 된 경우인데 */
                fprintf(stderr, "I don't want to handle this case1\n");
            ...
            if(ntohs(data->length) == rev)
            {
                fprintf(stderr, "This is right case\n");
            }
            else
            {
                fprintf(stderr, "I don't want to handle this case2\n");
                break;
            }
            datalist[data_num] = data;
            data_num++;
            if(data_num >= datalist_size)
            {
                datalist = (struct protocol **)realloc(datalist,datalist_size+MB);
                datalist_size = datalist_size+MB;
            }
            rec_len -= rev-8;
        }
        printf("%s\n",datalist[0]->message);
